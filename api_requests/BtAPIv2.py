...
class BtV2:
    def __init__(self, bt_panel=None, bt_key=None, proxy=None):
        ...
        self.config = {
            # 自定义处理类
            'Public_config': '/v2/panel/public/get_public_config',  #: 读取面板设置
            'getSiteCat': '/v2/site?action=get_site_types',  #: 获取站点分类列表
            'selectCat': '/datalist/data/get_data_list',  #: 获取站点选择分类列表
            'uploadFile': '/v2/files?action=upload',  #: 文件上传
            'CopyFile': '/v2/files?action=CopyFile',  #: 复制文件
            'UnZip': '/v2/files?action=UnZip',  #: 解压
            'addSites': '/v2/site?action=create_website_multiple',  #: 批量添加网站
            'newFile': '/v2/files?action=CreateFile',  #: 新建文件
            'SaveFileBody': '/v2/files?action=SaveFileBody',  #: 保存or修改文件or伪静态规则内容(保存文件内容)
            'DeleteFile': '/v2/files?action=DeleteFile',  #: 删除文件
            'MvFile': '/v2/files?action=MvFile',  #: 移动文件 or 重命名
            'setSiteType': '/v2/site?action=set_site_type',
            'GetDirNew': '/v2/files?action=GetDirNew',  # 搜索文件名
            'SetFileAccess': '/v2/files?action=SetFileAccess',  # 设置文件权限
            'AddIpWhiteList': '/v2/config?action=set_token',  # 添加ip至白名单
            'GetDir': '/v2/files?action=GetDir',  # 获取目录下所有文件/文件夹
            'GetData': '/v2/data?action=getData',
            'AddSiteType': '/v2/site?action=add_site_type',  # 添加分类
            'AddDatabase': '/v2/database?action=AddDatabase',  # 添加Database
            'InputSql': '/v2/database?action=InputSql',  # 导入MYSQL
            'RemoveSitesType': '/v2/site?action=remove_site_type',  # 删除网站分类
            'GetFileBody': '/v2/files?action=GetFileBody',  #
            'DownloadFile': '/v2/files?action=DownloadFile',  # 下载 -> 加入到下载队列中
            'GetTaskList': '/v2/task?action=get_task_lists',  # 获取下载队列
            'SetToken': '/v2/config?action=set_token',  # 更新Key
            'GetCertList': '/v2/ssl?action=get_cert_list',  # SSL证书
            'GetSiteDomains': '/v2/site?action=GetSiteDomains',  # 获取一个域名下的子域名
            'ApplyCertApi': '/v2/acme?action=apply_cert_api',  # 申请SSL证书
            'GetRedirectList': '/v2/site?action=GetRedirectList',  # 获取重定向列表
            'CreateRedirect': '/v2/site?action=CreateRedirect',  # 创建重定向表
        }
    ...
    def http_post_cookie(self, url, data, timeout=60):
        ...
        for i in range(3):
            try:
                print(url)
                response = requests.post(url, headers=header, data=data, files=data, timeout=timeout, verify=False, proxies=self.proxy).json()
                print(33333, response)
            except Exception as e:
                time.sleep(0.5)
                logging.error(e, exc_info=True)
            else:
                response['status'] = False if response.get('status') else True
                return response

        # 解析JSON数据
        return {'status': False, 'msg': f'宝塔请求失败 url: {url} 请检查网络或者域名是否解析！'}
    ...
    def common_request(self, params, config, default_params, preprocess=None):
        ...
        try:
            ...
            # print(result.text)
            print(result)
            if result.get('message') and type(result['message']) == dict:
                result.update(result['message'])
            return result
        except Exception as e:
            return {'error': str(e)}
    ...
    def upload_file(self, params={}, cover=False):
        ...
        if not cover:  # 如果不覆盖的话，检查是否已存在
            ...
            data = self.http_post_cookie(url, p_data)
            # 如果文件存在，取消上传任务
            if data['status']:
                return data
            ...
            params['blob'] = (params['f_name'], open(os.path.join(params['blob']), 'rb+'), 'application/octet-stream')
        return self.http_post_cookie(url, {**self.get_key_data(), **params})
    ...
    def unZip(self, params={}):
        ...
        params = {**default_params, **params}
        res =  self.common_request(params, "UnZip", default_params)
    ...
    def add_database(self, params={}):
        default_params = {'name': '', 'db_user': '', 'password': '', 'dataAccess': '127.0.0.1', 'address': '127.0.0.1',
                          'codeing': 'utf8mb4',
    ...
    def create_redirection_list(self, params={}):
        default_params = {'type': '1', 'holdpath': '1', 'domainorpath': '', 'redirecttype': '', 'tourl': '',
                          'redirectdomain': [], 'sitename': '', 'redirectname': str(int(time.time() * 1000))}
        params = {**default_params, **params}
