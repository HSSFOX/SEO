...
class PerplexityModel:
    ...
    def main(self):
        ...
        self.msg_trigger.emit("任务已结束！", '', '')


    def thread_wrapper(self, word):
        if self.running_sign:
            self.ui.redis.r.lpop(f'{self.ui.main_page.username}_AiKeywordsQueue')       # 仅pop出当前线程中的那一个 而不是全部一次性(也时一个一个)排出
