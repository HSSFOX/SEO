...
from backend.AI_write.perplexity_model import PerplexityModel, PerplexityCheckKey
...
class AIWrite(Ui_Form):
    ...
    def get_redis_key_words(self):
        self.online_keywords = self.redis.r.lrange(f"{self.main_page.username}_AiKeywordsQueue", 0, -1)
        print(555555555, self.online_keywords)
    ...
    def get_combo_set(self):
        ...
        print(11111111111, self.api_refer_d)
    ...
    def start_task(self):
        if not self.default_start:      # 如果还没开始, 就变为已开始状态
            ...
            if run_sign:
                ...
                self.online_keywords = keywords
                self.redis.r.ltrim(f"{self.main_page.username}_AiKeywordsQueue", -1, 0)      # 清空redis
                print("now redis: ", self.redis.r.lrange(f"{self.main_page.username}_AiKeywordsQueue", 0, -1))
                self.redis.r.rpush(f"{self.main_page.username}_AiKeywordsQueue", *keywords)  # 插入redis
                print(f"{self.main_page.username}_AiKeywordsQueue")
                print(keywords)
                ...
                if self.comboBox_3.currentText() == '通义千问':
                    ...
                    model = self.comboBox_4.currentText()       # 模型选择
                    self.main_page.return_msg_update(f"正在使用 {gpt} {model} AI模型...")
                elif self.comboBox_3.currentText() == 'Perplexity':
                    gpt = 'Perplexity'
            ...
            self.pushButton_6.setText("任务终止中...")
            self.pushButton_6.setEnabled(False)
    ...
    def return_ai_write_msg(self, msg, word, title):
        if '错误信息' in msg:
            ...
            if word in self.online_keywords:
                self.online_keywords.remove(word)
                self.textEdit.setText("\n".join(self.online_keywords))
...
class ModelThread(QThread):
        ...
        if self.gpt == '通义千问':
            ...
            if [model_l[ele]['agent'] for ele in model_l][0]:
                ...
                self.chatgpt_model = ChatGptModel(self.ui, model_l, self.msg_trigger, self.output_trigger,
                                                  ...
                                                  self.model)
                self.chatgpt_model.main()
        elif self.gpt == 'Perplexity':

            if [model_l[ele]['agent'] for ele in model_l][0]:
                status, proxy_type, proxy = self.read_proxy()
                if status:
                    self.perplexity_model = PerplexityModel(self.ui, model_l, self.msg_trigger, self.output_trigger, self.request_config, self.content_config, self.keywords, self.keys, self.model, proxy_type, proxy)
                    self.perplexity_model.main()
                else:
                    self.ui.default_start = False
                    self.msg_trigger.emit("未检测到VPN设置, 请先前往代理设置", "", "")
            else:
                self.perplexity_model = PerplexityModel(self.ui, model_l, self.msg_trigger, self.output_trigger,
                                                  self.request_config, self.content_config, self.keywords, self.keys,
                                                  self.model)
                self.perplexity_model.main()
    ...
    def stop_threading(self):
        if self.gpt == '通义千问':
            ...
            self.perplexity_model.running_sign = False
    ...
    def run(self):
        ...
        if self.gpt == '通义千问':
            ...
            if [model_l[ele]['agent'] for ele in model_l][0]:
                status, proxy_type, proxy = self.read_proxy()
                if status:
                    PerplexityCheckKey(model_l, self.gpt, self.model, self.msg_trigger, proxy_type, proxy).main()
                else:
                    self.msg_trigger.emit("未检测到VPN设置, 请先前往代理设置")
            else:
                PerplexityCheckKey(model_l, self.gpt, self.model, self.msg_trigger).main()
        self.finished_trigger.emit()
