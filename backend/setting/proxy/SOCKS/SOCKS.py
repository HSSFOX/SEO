from frontend.setting.proxy.SOCKS.socks import Ui_Form
import requests
import configparser
...
class SOCKS(Ui_Form):
    ...
    def check_connection(self):
        ...
        if connection_info['username'] and connection_info['password']:
            proxies = {
                "http": f"socks5://{connection_info['username']}:{connection_info['password']}@{connection_info['host']}:{connection_info['port']}",
                "https": f"socks5://{connection_info['username']}:{connection_info['password']}@{connection_info['host']}:{connection_info['port']}",
            }
        else:
            proxies = {
                "http": f"socks5://{connection_info['host']}:{connection_info['port']}",
                "https": f"socks5://{connection_info['host']}:{connection_info['port']}",
            }
        # proxies = {
        #     "http": f"socks5://127.0.0.1:33211",
        #     "https": f"socks5://127.0.0.1:33211",
        # }
    ...
    def set_init_values(self):
        ...
        try:
            config.read('config/config.ini')  # 读取 config.ini 文件
            if config.has_section('Proxy') and config.get('Proxy', 'type') == 'socks':
